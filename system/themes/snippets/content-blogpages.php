...
<div class="entry-meta"><p><i class='fa fa-calendar'></i> <?php echo htmlspecialchars($page->getDate("published")) ?></p> <!--<?php echo $yellow->text->getHtml("blogBy") ?>-->
  ...
  <p><i class='fa fa-tag'></i> <?php $tagCounter = 0; foreach(preg_split("/,\s*/", $yellow->page->get("tag")) as $tag) { if(++$tagCounter>1) echo ", "; echo "<a href=\"".$yellow->page->getPage("blogpages")->getLocation(true).$yellow->toolbox->normaliseArgs("tag:$tag")."\">".htmlspecialchars($tag)."</a>"; } ?></p>
  <?php endif ?></p></div>
